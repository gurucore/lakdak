import {exec as $a9KXw$exec, spawn as $a9KXw$spawn} from "child_process";
import {promisify as $a9KXw$promisify} from "util";



const $2fed4d0d0c995d17$var$execAsync = (0, $a9KXw$promisify)((0, $a9KXw$exec));
class $2fed4d0d0c995d17$export$73e2f76829f803fa {
    /** undefined will be ""
...
It's not as comprehensive as "npm:shell-quote" but handles most common cases safely and is much lighter. */ static escapeArg(arg) {
   ...
   */ static async exec(command, args = [], hint, options = {}, bashCommandExtra = '') {
        ...
        const fullCommand = `${command} ${args.map($2fed4d0d0c995d17$export$73e2f76829f803fa.escapeArg).join(' ')} ${bashCommandExtra}`;
        if (!silent) console.log(`Executing (${hint ? hint : ''}): ${fullCommand}`);
        try {
            const { stdout: stdout, stderr: stderr } = await $2fed4d0d0c995d17$var$execAsync(fullCommand, options);
            return {
                output: stdout.trim(),
                error: stderr.trim(),
                hint: hint
   ...
   */ static async spawn(command, args = [], hint, options = {}) {
        ...
        const childProc = (0, $a9KXw$spawn)(command, args, {
            // env,
            // cwd,
            ...spawnOptions
        });
...
export {$2fed4d0d0c995d17$export$73e2f76829f803fa as CLIHelper};
//# sourceMappingURL=CLIHelper.54de6e03.js.map
